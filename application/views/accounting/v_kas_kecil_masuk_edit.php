        ...
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                ...
                <section class="content">
                    <div class="box">
                        <form class="form-horizontal" method="POST" name="form-acc-kaskecil" id="form-acc-kaskecil" action="<?= base_url("accounting/kaskecilmasuk/update/{$id}") ?>">
                            <input type="hidden" value="<?= $datas->id ?>" name="ids">
                            <div class="box-header with-border">
                                <h3 class="box-title"><strong><?= $datas->no_kkm ?></strong></h3>
            ...
            <template class="kaskeluar-tmplt">
                <tr class="add-fpt-tr list-new">
                    ...
                    <td>
                        <input type="text" name="nominal[]" pattern="^-?\d{1,3}(,\d{3})*(\.\d+)?$" data-type='currency' class="form-control nominal nominal:nourut text-right input-sm" value="0" required/>
                    </td>
                </tr>
            </template>
            <script>
...
//                        }
                ...
                $(function () {
                    ...
                    const updateStatus = ((statuss) => {
                        $.ajax({
                            url: "<?= base_url("accounting/kaskecilmasuk/update_status/{$id}") ?>",
                    ...
                    $("#btn-confirm").unbind("click").off("click").on("click", function (e) {
                        ...
                        confirmRequest("Kas Kecil Masuk", statuss + " Data ? ", (() => {
                    ...
                    $("#btn-draft").unbind("click").off("click").on("click", function (e) {
                        e.preventDefault();
                        confirmRequest("Kas Kecil Masuk", "Simpan Kembali Sebagai Draft ? ", (() => {
                            updateStatus("draft");
                        }));
                    });
                    ...
                    $("#btn-print").on("click", function (e) {
                        e.preventDefault();
                        $.ajax({
                            url: "<?= base_url('accounting/kaskecilmasuk/print') ?>",
                            type: "POST",
                            data: {
                                no: "<?= $id ?>"
                            },
                    ...
                    $.ajax({
                        url: "<?= base_url('accounting/kaskecilmasuk/print_pdf/') ?>",
                        type: "POST",
                        data: {
                            id: "<?= $id ?>"
                        },
