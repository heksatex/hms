...
//      $menu        = $this->m_menu->sub_main_menu($username,$this->uri->segment(1));//mengambil data sub main menu
        ...
        foreach ($menuss[$this->uri->segment(1)] as $menus):
            if ($this->session->userdata("{$menus->kode}_{$username}")) {
                $sub_menu = unserialize($this->session->userdata("{$menus->kode}_{$username}"));
            } else {
                $sub_menu = $this->db->order_by('mms.row_order', 'ASC')->JOIN('user_priv as up', 'mms.kode = up.main_menu_sub_kode', 'INNER')
                                ->get_where('main_menu_sub as mms', array('mms.is_menu_sub' => $menus->kode, 'up.username' => $username))->result();
                $this->session->set_userdata("{$menus->kode}_{$username}", serialize($sub_menu));
            }
...
//        $this->db->order_by('mms.row_order','ASC');
            ...
            if (count($sub_menu) > 0) {// sub menu dengan child nya
