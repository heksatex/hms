    ...
    <body>
        ...
        <div class="d-flex p-2">
            ...
            <nav class="sidebar d-flex flex-column flex-shrink-0 position-fixed">
                ...
                <div class="panel-box hide-on-collapse">
                    <div class="panel-title text-dark">
                        <span><i class="fa-solid fa-list-check text-warning me-2"></i>Daftar MO Warping</span>
                        <span class="badge bg-warning text-dark" id="mo-count">0 MO</span>
                    </div>

                    <!-- SEARCH & MULTI-FILTER ELEMENTS -->
                    <div class="mb-2 d-flex flex-column gap-1">
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" id="searchInputMO" class="form-control border-start-0 ps-0" placeholder="Cari MO, Produk, MC, GB...">
                        </div>

                        <div class="row g-1">
                            <div class="col-6">
                                <select id="filterStatus" class="form-select form-select-sm" onchange="moList()">
                                    <option value="unassigned">Belum Terjadwal</option>
                                    <option value="" selected>Semua MO</option>
                                    <option value="assigned">Sudah Terjadwal</option>
                                </select>
                            </div>
                            <div class="col-6">
                                <select id="filterMachine" class="form-select form-select-sm" onchange="moList()">
                                    <option value="">Semua MC Target</option>
                    ...
                    <div class="mo-scroll-container" id="mo-list">
                        <!-- MO ITEMS RENDERED HERE VIA JS -->
                    </div>
                </div>
            </nav>


            <!-- 2. ADVANCED GANTT CHART TIMELINE -->
            <main class="main-content">
                <div class="container-fluid">
                    <div class="panel-title text-dark">
                        ...
                        <div class="mo-card" style="border-left: 4px solid #1B13F5"
                             draggable="true" data-total_minute="30" data-tipe="break" data-nama_produk="Ganti Lembar" ondragstart="dragStartTimeBreak(event)" ondragend="dragEnd(event)">
                            <span style="color: #1B13F5"
                                  >Ganti Lembar</span>
                        </div>
                        <div class="mo-card" style="border-left: 4px solid #484858"
                             draggable = "true" data-total_minute="120" data-tipe="break" data-nama_produk="Bongkar Pasang" ondragstart="dragStartTimeBreak(event)" ondragend="dragEnd(event)">
                            <span style="color:#484858">
                                Bongkar Pasang</span>  
                        </div>
        ...
        <!-- MODAL MASTER BENANG AKTIF PER MESIN -->
        <div class="modal fade" id="yarnMasterModal" tabindex="-1" aria-labelledby="yarnMasterModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-l modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header bg-teal text-white py-2">
                        <h6 class="modal-title fw-bold" id="yarnMasterModalLabel"><i class="fa-solid fa-boxes-stacked me-2"></i>Master Benang Aktif & Tanggal Naik Mesin</h6>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-content p-3">
                        <p class="xsmall text-muted mb-2">Inputkan jenis benang, sisa stok benang (Kg), dan <strong>Tanggal & Waktu Naik</strong> benang ke mesin. Estimasi habis hanya dihitung untuk MO yang dikerjakan <strong>setelah tanggal/waktu naik tersebut</strong>.</p>
                        <form class="form-horizontal" method="POST" name="form-edit-mesin" id="form-edit-mesin" action="<?= base_url("ppic/productionplanning/updatemesin") ?>">
                            <table class="table table-sm table-bordered align-middle">
                                <tbody class="table-light text-center xsmall">
                                    <tr>
                                        <td>Mesin</td>
                                        <td><input type="text" name="mc[]" id="frm-mc" class="form-control form-control-sm" readonly></td>
                                    </tr>
                                    <tr>
                                        <td>Nama / Kode Benang Aktif</td>
                                        <td> <input type="text" name="frm-bng[]" id="frm-bng" class="form-control form-control-sm" placeholder="Contoh: SDY 50-36 SDC"></td>
                                    </tr>
                                    <tr>
                                        <td>Stok Benang (Kg)</td>
                                        <td> <input type="number" name="frm-qty[]" id="frm-qty" class="form-control form-control-sm text-end" min="0" step="10"></td>
                                    </tr>
                                    <tr>
                                        <td>Waktu Naik Benang</td>
                                        <td> <input type="text" id="frm-time" name="frm-time[]" class="form-control form-control-sm tanggal-bng" value="<?= date("Y-m-d H:i") ?>"></td>
                                    </tr>
                                    <tr>
                                        <td>Estimasi Kapan Habis</td>
                                        <td class="info-est">
        ...
        <div class="modal fade" id="yarnMasterModalAll" tabindex="-1" aria-labelledby="yarnMasterModalAllLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-scrollable">

                <div class="modal-content">
                    <div class="modal-header bg-teal text-white py-2">
                        <h6 class="modal-title fw-bold" id="yarnMasterModalAllLabel"><i class="fa-solid fa-boxes-stacked me-2"></i>Master Benang Aktif & Tanggal Naik Mesin</h6>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-content p-3">
                        <p class="xsmall text-muted mb-2">Inputkan jenis benang, sisa stok benang (Kg), dan <strong>Tanggal & Waktu Naik</strong> benang ke mesin. Estimasi habis hanya dihitung untuk MO yang dikerjakan <strong>setelah tanggal/waktu naik tersebut</strong>.</p>
                        <form class="form-horizontal" method="POST" name="form-edit-mesin-all" id="form-edit-mesin-all" action="<?= base_url("ppic/productionplanning/updatemesin") ?>">
                            <table class="table table-sm table-bordered align-middle">
                                <thead class="table-light text-center xsmall">
                                    <tr>
                                        <th style="width: 110px;">Mesin</th>
                                        <th>Nama / Kode Benang Aktif</th>
                                        <th style="width: 130px;">Stok Benang (Kg)</th>
                                        <th style="width: 200px;">Waktu Naik Benang</th>
                                        <th style="width: 200px;">Estimasi Kapan Habis</th>
                                    </tr>
                                </thead>
                                <tbody id="yarn-master-tbody">
                                    <!-- POPULATED VIA JS -->
                                </tbody>
                            </table>
                            <button class="btn btn-sm btn-teal text-white btn-edit-mesin-all" style="background-color: var(--teal);">Simpan Master Benang</button>
                        </form>
...
//                                            document.getElementById('zoom-level-text').innerText = `${currentZoomLevel}%`;
            ...
            <span class="mo-code">${mo.kode} </span><span class="badge ${(mo.status === 'draft') ? 'bg-warning text-dark' : 'bg-success'} text-capitalize" style="font-size:0.6rem;">${mo.status}</span>
                ...
                <div>
                    ${(mo.total_minute != '0') ? `<span class="badge bg-success me-1" style="font-size:0.6rem">${mo.mc_id}</span>` : ''}
                    <span class="badge bg-secondary text-white" style="font-size:0.65rem" title="Target Mesin Warping">
                        <i class="fa-solid fa-industry me-1"></i>${(mo.total_minute != '0') ? mo.mc : mo.mc_id}
                    </span>
                </div>
            </div>

            <div class="text-truncate fw-bold text-dark mb-1" style="font-size:0.75rem" title="${mo.nama_produk}">
                ${mo.nama_produk}
            </div>

            <div class="d-flex justify-content-between align-items-center text-muted xsmall mb-1">
                <span><i class="fa-solid fa-cube me-1 text-primary"></i>${Number(mo.qty).toLocaleString()} ${mo.uom}</span>
                <span class="text-dark fw-bold" title="Lama Pengerjaan">
                    <i class="fa-regular fa-clock me-1 text-warning"></i>${diffInMinutes}m (${durHoursFormatted}h)
                </span>
            </div>

            <div class="d-flex justify-content-between align-items-center pt-1 border-top text-muted" style="font-size:0.65rem;">
                <span><i class="fa-solid me-1 text-info"></i>${hslnm[1] ?? ''} ${hslnm[2] ?? ''}</span>
                                        ...
                                        mesinDataVis.forEach(mc => {
                                            ...
                                            tr.innerHTML = `
                                                       <td class="fw-bold text-center">${mc.id}
                                                                   ...
                                                                   <td>
                                                                    <input type="text" id="yarn-name-${mc.id}" name="frm-bng[]" class="form-control form-control-sm" value="${mc.benang}" placeholder="Contoh: SDY 50-36 SDC">
                                                    </td>
                                                                        <td>
                        <input type="number" id="yarn-qty-${mc.id}" class="form-control form-control-sm text-end" name="frm-qty[]" value="${mc.qty}" min="0" step="10">
                    </td>
                            <td>
                        <input type="text" class="form-control form-control-sm tanggal-bng" name="frm-time[]" value="${mc.time}">
                    </td>
