...
class Kaskecilkeluar extends MY_Controller {
    ...
    public function index($depth = "ACCKKK") {
        $data['id_dept'] = $depth;
    ...
    protected function _list_data() {
        try {
            ...
            if ($status !== "") {
                $list->setWheres(["acc_kas_kecil_keluar.status" => "{$status}"]);
            }
            if ($tanggal !== "") {
    ...
    public function add($depth = "ACCKKK") {
        $data['id_dept'] = $depth;
        $data["class"] = $this->uri->segment(1);
        $model = new $this->m_global;
//        $data["coas"] = $model->setTables("acc_coa")->setSelects(["kode_coa", "nama"])
//                        ->setWheres(["level" => 5])->setOrder(["kode_coa" => "asc"])->getData();
        $data["coa"] = $model->setTables("acc_coa")->setWheres(["nama" => "Kas Kecil",'status'=>'aktif'])->setOrder(["kode_coa" => "asc"])->getData();
    ...
    public function edit($id,$depth = "ACCKKK") {
        try {
            $data["class"] = $this->uri->segment(1);
