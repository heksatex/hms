...
class Kasmasuk extends MY_Controller {
    ...
    public function index($depth = "ACCKM") {
        $data['id_dept'] = $depth;
    ...
    public function ekspor() {
        try {
            ...
            foreach ($data->getData() as $key => $field) {
                ...
                $sheet->setCellValue("B{$row}", $field->no_km);
                $sheet->setCellValue("C{$row}", $customer);
    ...
    public function list_data() {
        try {
            ...
            foreach ($list->getData() as $field) {
                ...
                $data [] = [
                    $no,
                    "<a href='" . base_url("{$class}/kasmasuk/edit/{$kode_encrypt}") . "'>{$field->no_km}</a>",
    ...
    public function add($depth = "ACCKM") {
        $data['id_dept'] = $depth;
        $data["class"] = $this->uri->segment(1);
...
//        $data["no"] = $nokm;
    ...
    public function simpan() {
        ...
        try {
            ...
            $url = site_url("{$class}/kasmasuk/edit/" . encrypt_url($nokm));
            $this->output->set_status_header(200)
                    ->set_content_type('application/json', 'utf-8')
    ...
    public function edit($id,$depth = "ACCKM") {
        try {
    ...
    public function add_data_from_tarik_tunai() {
        try {
            ...
            $data = $model->setTables("acc_giro_keluar_detail agkd")->setJoins("acc_giro_keluar agk", "agkd.no_gk = agk.no_gk")
                            ...
                            ->setSelects(["agkd.nominal,agkd.no_gk,agkd.kode_coa,agkd.bank,agkd.no_rek,agkd.no_bg", "agkd.id,if(partner_nama = '',agk.lain2,partner_nama) as lain"])
...
//            ];
            ...
            $url = site_url("{$class}/kasmasuk/edit/{$id}");
            $this->output->set_status_header(200)
                    ->set_content_type('application/json', 'utf-8')
                    ->set_output(json_encode(array('message' => 'Berhasil', 'icon' => 'fa fa-check', 'type' => 'success', 'url' => $url)));
        } catch (Exception $ex) {
...
//            $printer->feed();
    ...
    public function print_pdf() {
        try {
            ...
            $head = $model->setTables("acc_kas_masuk")->setJoins("acc_coa", "acc_coa.kode_coa = acc_kas_masuk.kode_coa")
                            ->setSelects(["acc_kas_masuk.*", "acc_coa.nama as nama_coa","date(tanggal) as tanggal"])
                            ->setWheres(["no_km" => $kode])->getDetail();
            if (!$head) {
                throw new \exception("Data No Kas Masuk {$kode} tidak ditemukan", 500);
            }
            $data["detail"] = $model->setTables("acc_kas_masuk_detail")
                            ->setJoins("currency_kurs", "currency_kurs.id = currency_id")
                            ->setWheres(["kas_masuk_id" => $head->id])
                            ->setSelects(["acc_kas_masuk_detail.*", "currency_kurs.currency as curr"])->getData();
            $data["head"] = $head;
            $html = $this->load->view("print/acc/v_kas_masuk_print", $data, true);
            ...
            $mpdf->WriteHTML($html);
            $filename = str_replace("/", "-", $data["head"]->no_km);
