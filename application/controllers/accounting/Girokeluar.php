...
class Girokeluar extends MY_Controller {
    ...
    public function index($depth = 'ACCGK') {
        $data['id_dept'] = $depth;
    ...
    protected function _list_data() {
        try {
            ...
            $list->setTables("acc_giro_keluar")->setOrder(["acc_giro_keluar.tanggal" => "desc","no_gk"=>"desc"])
...
public function ekspor() {
        try {
            ...
            $sheet->setCellValue("G{$row}", 'Transinfo');
            ...
            foreach ($data->getData() as $key => $field) {
                ...
                $sheet->setCellValue("B{$row}", $field->no_gk);
                ...
                $sheet->setCellValue("G{$row}", $field->no_bk2);
                $sheet->setCellValue("H{$row}", $field->total_rp);
                $sheet->setCellValue("I{$row}", $field->status);
            }
            if($noUrut > 0) {
                $sheet->getStyle("H4:H{$row}")->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
            }
            $filename = "Giro Keluar ".date("Y-m-d");
    ...
    public function add($depth = 'ACCGK') {
        $data['id_dept'] = $depth;
        $data["class"] = $this->uri->segment(1);
...
//                        ->setWheres(["level" => 5])->setOrder(["kode_coa" => "asc"])->getData();
        $data["coa"] = $model->setTables("acc_coa")->setWhereIn("jenis_transaksi", ["utang_giro", "piutang_giro"])->setWheres(['status'=>'aktif'])->setOrder(["kode_coa" => "asc"])->getData();
    ...
    public function edit($id,$depth = 'ACCGK') {
        try {
...
//                    $checkKasMasuk = $model
    ...
    public function print_pdf() {
        try {
            ...
            $data["detail"] = $model->setTables("acc_giro_keluar_detail")
                            ->setJoins("currency_kurs", "currency_kurs.id = currency_id")
                            ->setWheres(["giro_keluar_id" => $head->id])
                            ->setSelects(["acc_giro_keluar_detail.*", "currency_kurs.currency as curr"])->getData();
            $data["head"] = $head;
            $html = $this->load->view("print/acc/v_giro_keluar_print", $data, true);
            ...
            $mpdf->WriteHTML($html);
            $filename = str_replace("/", "-", $data["head"]->no_gk);
