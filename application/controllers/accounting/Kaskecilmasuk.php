...
class Kaskecilmasuk extends MY_Controller {
    ...
    protected function _list_data() {
        try {
            ...
            $list->setTables("acc_kas_kecil_masuk")->setOrder(["acc_kas_kecil_masuk.tanggal" => "desc","no_kkm"=>"desc"])
                    ...
                    ->setOrders([null, "no_kkm", "partner_nama", "acc_kas_kecil_masuk.tanggal", "acc_coa.kode_coa", "total_rp"])
            ...
            if ($status !== "") {
                $list->setWheres(["acc_kas_kecil_masuk.status" => "{$status}"]);
            }
            if ($tanggal !== "") {
    ...
    public function ekspor() {
        try {
            ...
            foreach ($data->getData() as $key => $field) {
                ...
                $sheet->setCellValue("B{$row}", $field->no_kkm);
                $sheet->setCellValue("C{$row}", $customer);
            ...
            $filename = "Kas Kecil Masuk ".date("Y-m-d");
...
//            $this->validasiPin($pin, "Edit Data Hanya bisa dilakukan Oleh Supervisor", $dt->tanggal);
            ...
            $asalDetail = $model->setTables("acc_kas_kecil_masuk_detail")->setWheres(["no_kkm" => $kode])->getData();
            $model->delete();
            $detail = [];
            if (count($kodeCoa) > 0) {
                $kurs = $this->input->post("kurs");
            ...
            switch ($status) {
                case "confirm":
                    ...
                    if ($head->jurnal !== "") {
                        ...
                        if (!$jurnal = $this->token->noUrut("jurnal_acc_kkm", date('y', strtotime($head->tanggal)) . '/' . date('m', strtotime($head->tanggal)), true)
    ...
    public function print_pdf() {
        try {
            ...
            $head = $model->setTables("acc_kas_kecil_masuk")->setJoins("acc_coa", "acc_coa.kode_coa = acc_kas_kecil_masuk.kode_coa")
                            ->setSelects(["acc_kas_kecil_masuk.*", "acc_coa.nama as nama_coa","date(tanggal) as tanggal"])
                            ->setWheres(["no_kkm" => $kode])->getDetail();
            if (!$head) {
                throw new \exception("Data No Kas Kecil Masuk {$kode} tidak ditemukan", 500);
            }
            $data["detail"] = $model->setTables("acc_kas_kecil_masuk_detail")
                            ->setJoins("currency_kurs", "currency_kurs.id = currency_id")
                            ->setWheres(["kas_kecil_masuk_id" => $head->id])
                            ->setSelects(["acc_kas_kecil_masuk_detail.*", "currency_kurs.currency as curr"])->getData();
            $data["head"] = $head;
            $html = $this->load->view("print/acc/v_kas_kecil_masuk_print", $data, true);
            ...
            $mpdf->WriteHTML($html);
            $filename = str_replace("/", "-", $data["head"]->no_kkm);
