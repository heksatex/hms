...
class Adjustment extends MY_Controller
{
  ...
  public function list_import_produk()
  {
      ...
      foreach ($list as $field) {
          ...
          $row[] = $field->kode_produk."|".htmlentities($field->nama_produk)."|".$field->lot."|".$field->qty."|".$field->uom."|".$field->qty2."|".$field->uom2."|".$field->lokasi."|".$field->quant_id."|".$field->reserve_move."|";
          $data[] = $row;
      }
      $output = array(
          "draw" => $_POST['draw'],
  ...
  { 
    ...
    if (empty($this->session->userdata('status'))) {//cek apakah session masih ada
      ...
      //lock tabel
      $this->_module->lock_tabel('adjustment WRITE, adjustment_items WRITE');
      ...
      if(!empty($cek1['status'])){
        ...
        for($i=0; $i <= $countchek-1;$i++){
          ...
          $row_data = explode("|", $dt1);

          $kode_produk = $row_data[0];
          $nama_produk = $row_data[1];
          $lot         = $row_data[2];
          $qty         = $row_data[3];
          $uom         = $row_data[4];
          $qty2        = $row_data[5];
          ...
          if(empty($cek_quant['kode_adjustment'])){
            ...
            $sql_adjustment_items_batch .= "('".$kode_adjustment."', '".$quant_id."', '".addslashes($kode_produk)."','".addslashes($lot)."', '".addslashes($uom)."','".$qty."','".$qty."', '".addslashes($uom2)."','".$qty2."','".$qty2."', '".$row_order."'), ";
            $row_order++;            
          }else{
            $lot_sama .= $lot.', ';
        ...
        if($item_add == true){
          if(!empty($lot_sama)){
            ...
            $callback = array('status'=>'success',  'message' => 'Adjustment Detail Berhasil Ditambahkan !',  'icon' =>'fa fa-check', 'type' => 'success', 'msg2'=>'Yes', 'message2'=> 'Produk ( '.$lot_sama.' )</br> Sudah Pernah Diinput !'); 
          }else{
          ...
          $lot_sama = rtrim($lot_sama, ', ');
          $callback = array('status'=>'failed',  'message' => 'Produk ( '.$lot_sama.' )</br> Sudah Pernah Diinput !',  'icon' =>'fa fa-check', 'type' => 'success'); 
  ...
  public function generate_detail_adjustment_items()
  {
    ...
    if(empty($this->session->userdata('username'))){
      ...
      if(!empty($cek1['status'])){
        ...
        // lock table
        $this->_module->lock_tabel('stock_move WRITE, stock_move_items WRITE, stock_move_produk WRITE, stock_quant WRITE, adjustment WRITE, adjustment_items WRITE, mst_produk WRITE, departemen WRITE, adjustment_items ai WRITE, mst_produk mp WRITE, user WRITE, log_history WRITE, main_menu_sub WRITE ');
        ...
        foreach($item as $row){
          ...
          if($row->quant_id == 0){
              $loop_adj = true;
              // simpan stock_quant
              $sql_stock_quant_batch .= "('".$start."','".$tanggal."','".addslashes($row->kode_produk)."','".addslashes($row->nama_produk)."','".addslashes($row->lot)."','','".$row->qty_adjustment."','".$row->uom."','".$row->qty_adjustment2."','".$row->uom2."','".$kode_lokasi."','','',''), ";
              ...
              $sql_stock_move_items_batch .= "('".$move_id."', '".$start."','".addslashes($row->kode_produk)."', '".addslashes($row->nama_produk)."','".addslashes(trim($row->lot))."','".$row->qty_adjustment."','".($row->uom)."','".$row->qty_adjustment2."','".$row->uom2."','".$status_done."','1','','".$tanggal."'), ";
              ...
              if($qty_data != $qty_adjustment){
                ...
                if($qty_data > $qty_adjustment){
                  ...
                  if($qty_adjustment != 0){
                    ...
                    $sql_stock_quant_batch .= "('".$start."','".$tanggal."','".addslashes($row->kode_produk)."','".addslashes($row->nama_produk)."','".addslashes($row->lot)."','".$nama_grade."','".$qty_adj."','".$row->uom."','".$row->qty_adjustment2."','".$row->uom2."','".$lokasi_tujuan."','".$reff_note."','','".$reserve_origin."'), ";
                ...
                // simpan stock_move_items
                $sql_stock_move_items_batch .= "('".$move_id."', '".$quant_id."','".addslashes($row->kode_produk)."', '".addslashes($row->nama_produk)."','".addslashes(trim($row->lot))."','".$qty_adj."','".$row->uom."','".$row->qty_adjustment2."','".$row->uom2."','".$status_done."','1','','".$tanggal."'), ";
        ...
        if($loop_adj == true){
          ...
          // update qty stock_quant
          if(!empty($case4) AND !empty($where4)){
            $where4 = rtrim($where4, ',');
            $sql_update_qty_stock_quant = "UPDATE stock_quant SET qty = (case ".$case4." end)  WHERE quant_id in (".$where4.") ";
          ...
          if(!empty($case5) AND !empty($where5)){
            $where5 = rtrim($where5, ',');
            $sql_update_lokasi_stock_quant = "UPDATE stock_quant SET lokasi = (case ".$case5." end)  WHERE quant_id in (".$where5.") ";
          ...
          if(!empty($case7) AND !empty($where7)){
            $where7 = rtrim($where7, ',');
            $sql_update_qty_move = "UPDATE adjustment_items SET qty_move = (case ".$case7." end) WHERE row_order in (".$where7.") AND kode_adjustment = '".$kode_adjustment."' ";
