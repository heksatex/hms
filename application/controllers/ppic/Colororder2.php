...
class Colororder extends MY_Controller
{
    ...
    function get_data()
    {
        $list = $this->m_colorOrder->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $field) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="'.base_url('ppic/colororder/edit/'.$field->kode_co).'">'.$field->kode_co.'</a>';
            $row[] = $field->tanggal;
            $row[] = $field->kode_sc;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->m_colorOrder->count_all(),
            "recordsFiltered" => $this->m_colorOrder->count_filtered(),
            "data" => $data,
        );
        ...
        if (empty($this->session->userdata('status'))) {//cek apakah session masih ada
          ...
          if(empty($kode_sc)){
              ...
              if(empty($kode_co)){//jika kode co kosong, aksinya simpan data
                  ...
                  $callback = array('status' => 'success', 'field' => 'kode_co' , 'message' => 'Data Berhasil Disimpan !', 'isi'=> $kode['kode_co'], 'icon' =>'fa fa-check', 'type' => 'success');

                  $jenis_log = "Create";
                  $note_log  =$kode['kode_co']."|".$route."|".$kode_sc."|".$buyer_code."|".$handling."|".$lbr_jadi."|".$tgl_sj."|".$note;
                  $this->m_colorOrder->gen_history($sub_menu, $kode['kode_co'], $jenis_log, $note_log, $username);
                  ...
                  $this->m_colorOrder->gen_history($sub_menu, $kode_co, $jenis_log, $note_log, $username);
    ...
    {   
        ...
        $data['id_dept']   ='CO';
        $data['detail']    = $this->m_colorOrder->get_data_detail_by_code($id);
        $data["colororder"] = $this->m_colorOrder->get_data_by_code($id);
    ...
    public function generate()
    {
      ...
      if (empty($this->session->userdata('status'))) {//cek apakah session masih ada
        ...
        //lock table
        $this->m_colorOrder->lock_tabel('mst_produk WRITE, mrp_route WRITE, mrp_route as mr WRITE, departemen WRITE, departemen as d WRITE, color_order_detail as cod WRITE,color_order_detail WRITE, color_order co WRITE, bom WRITE, bom_items WRITE, stock_move WRITE, stock_move_items WRITE, penerimaan_barang WRITE, penerimaan_barang_items WRITE, pengiriman_barang WRITE, pengiriman_barang_items WRITE, mrp_production WRITE, mrp_production_rm_target WRITE, mrp_production_fg_target WRITE');
        ...
        foreach ($cd as $val) {
          ...
          if(empty($cek_prod1['nama_produk'])){
            ...
            $sql_insert_batch .= "('".$kode_prod."', '".$prod."', '".$tgl."'), ";
        ...
        foreach ($cod as $val) {
            ...
            foreach ($route_prod as $rp) {
              ...
              if ($method_action == 'PROD'){
                ...
                if(empty($cek_prod2['nama_produk'])){
                  $sql_insert_batch .= "('".$kode_prod."','".$product_fullname."','".$tgl."'), ";
                  $last_number = $last_number + 1;
                }else{
              ...
              $sql_stock_move_items_batch .= "('".$move_id."','".$kode_prod_rm."','".$nama_prod_rm."','".$qty."','".$uom."','draft','1'), ";
              $sm_row = $sm_row + 1;
              ...
              if($method_action == 'OUT'){//Generate Pengiriman
                ...
                $sql_mrp_prod_batch .= "('".$kode_mo."','".$tgl."','".$origin."','".$kode_prod_rm."','".$nama_prod_rm."','".$qty."','m','".$tgl."','".$reff_notes."','".$kode_bom."','".$tgl."','".$tgl."','".$source_location."','".$source_location."','".$method_dept."','draft'), ";
          ...
          if(!empty($sql_stock_move_batch)){
            ...
            $this->m_colorOrder->create_stock_move_items_batch($sql_stock_move_items_batch);
          }
    ...
    public function get_data_sales_contract()
    {
        ...
        foreach ($list as $field) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = $field->nosc .' <a class="pilih" sale_contract="'.$field->nosc.'" buyer-code="'.$field->buyer_code.'"><span class="glyphicon glyphicon-check"></span></a>';
            $row[] = $field->buyer_code;
