...
class Bukubesarpembantuutangdetail extends MY_Controller
{
    ...
    {
        $data['id_dept'] = 'RBBPUD';
        $this->load->view('report/v_bukubesar_pembantu_utang_detail', $data);
    }
      ...
      function proses_data($checkhidden,$tgl_dari,$tgl_sampai,$partner,$currency)
    {
        ...
        $data = $this->m_bukubesarpembantuutang->get_list_bukubesar_detail($tgl_dari, $tgl_sampai, $checkhidden, $where_partner,$currency);
        ...
        foreach ($data as $datas) {
            ...
            $data2 = $this->m_bukubesarpembantuutang->get_list_bukubesar_detail_by_kode($tgl_dari,$tgl_sampai,$where2,$currency);
    ...
    function export_excel()
    {
        try {
            ...
            // set Judul
            $rowCount++;
            $sheet->SetCellValue('A'.$rowCount, 'BUKU BESAR PEMBANTU UTANG DETAIL');
            $sheet->mergeCells('A'.$rowCount.':G'.$rowCount);
            $sheet->getStyle('A'.$rowCount)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
