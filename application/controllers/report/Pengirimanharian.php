 ...
 * 
 */
class Pengirimanharian extends MY_Controller
{
    ...
    public function index()
	{
		...
		$this->load->view('report/v_pengiriman_harian', $data);
	}
	...
	function loadData()
	{
		...
		$list  = $this->m_inout->get_list_pengiriman_harian_by_kode($tgldari,$tglsampai,$departemen,$dept_tujuan,$status);
		$total = 0;
		foreach($list as $row){
	...
	function export_excel_out()
	{
    	...
    	// SET JUDUL
 		$object->getActiveSheet()->SetCellValue('A1', 'Laporan Pengiriman Harian');
		...
		$name_file ='Pengiriman Harian '.$dept['nama'].'.xls';
