...
class Bukubesarpembantupiutang extends MY_Controller
{
    ...
    function proses_data($tgldari, $tglsampai, $checkhidden)
    {
        ...
        $where      = ['p.customer' => 1];
        $data       = $this->m_bukubesarpembantupiutang->get_list_bukubesar($tgl_dari, $tgl_sampai, $checkhidden, $where);
        $tmp_data = array();
        $debit    = 0;
        $credit   = 0;
        $saldo_awal = 0;
        ...
        foreach ($data as $datas) {
            ...
            $tmp_data[] = array(
                ...
                'dpp_piutang' => (float) $datas->dpp_piutang,
                'ppn_piutang' => (float) $datas->ppn_piutang,
                'total_piutang_dpp_ppn' => (float) $datas->total_piutang_dpp_ppn,
                'pelunasan'   => $total_pelunasan,
                'retur'       => $total_retur,
                'dpp_retur' => (float) $datas->dpp_retur,
                'ppn_retur' => (float) $datas->ppn_retur,
                'total_retur_dpp_ppn' => (float) $datas->total_retur_dpp_ppn,
                'dpp_diskon' => (float) $datas->dpp_diskon,
                'ppn_diskon' => (float) $datas->ppn_diskon,
                'total_diskon_dpp_ppn' => (float) $datas->total_diskon_dpp_ppn,
