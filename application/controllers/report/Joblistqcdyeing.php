 ...
 * 
 */
class Joblistqcdyeing extends MY_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model('m_joblistqcdyeing');
	...
	public function index()
	{
		...
		$this->load->view('report/v_job_list_qc_dyeing', $data);
	}
	...
	public function get_data()
	{
        ...
        foreach ($list as $field) {
            ...
            $link_out        = '<a href="'.base_url('warehouse/pengirimanbarang/edit/'.$kode_out_mg_enc).'" target="_blank" data-toggle="tooltip" title="No Dye Out :'.$kode_out_mg.'"" '.$color4.'>'.$status_out.'</a> '.$info_backorder;
            ...
            // get_ move id out
            $mv = $this->m_joblistqcdyeing->get_move_id_by_sourve_move($move_id_fg)->row_array();
            $move_id =  $mv['move_id'];
            //get move_id out by move_id prod
            $outs = $this->m_joblistqcdyeing->get_pengiriman_barang_by_move_id($move_id)->row_array();
            $kode_out  = $outs['kode'];
            $qc1_out   = $outs['qc_1'];
            $qc2_out   = $outs['qc_2'];
            $qc        = $this->m_joblistqcdyeing->get_quality_control_by_kode($kode_out,$field->dept_id)->row();
    ...
    function cek_status_kain($kode,$category)
    {
        ...
        $rs = $this->m_joblistqcdyeing->cek_item_rm_target($kode,$category);
        $status = "Ready";
        $loop   = false;
        foreach($rs as $row){
            $loop = true;
    ...
    {
        // get kode outm mg 
        $kode     = $this->m_joblistqcdyeing->get_link_out_by_kode($origin,$method)->row_array();
        $kode_out = $kode['kode'];
