 ...
 * 
 */
class Joblistfinbrushing extends MY_Controller
{
	...
	public function index()
	{
        ...
        $data['id_dept_asli'] = 'FBR' ;
		$this->load->view('report/v_job_list_finbrushing', $data);
	}
	...
	public function get_data()
	{
        ...
        $id_dept_data = array('FBR','FBR-R');
        $list = $this->m_joblistfinishing->get_datatables($id_dept_data,$kode['kode']);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $field) {
            ...
            $link_kain      = '<a href="'.base_url('warehouse/penerimaanbarang/edit/'.$kode_in_kain_enc).'" target="_blank" data-toggle="tooltip" title="No FBR IN :'.$kode_in_kain.'"" '.$color.'>'.$alias_stat_kain.'</a>';
            ...
            $status_mg     = '<a href="'.base_url('manufacturing/mO/edit/'.$kode_encrypt).'" target="_blank" data-toggle="tooltip" title="MG FBR '.$kode_mrp.'" '.$color3.'>'.$nama_status.'</a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->m_joblistfinishing->count_all($id_dept_data,$kode['kode']),
            "recordsFiltered" => $this->m_joblistfinishing->count_filtered($id_dept_data,$kode['kode']),
            "data" => $data,
        );
