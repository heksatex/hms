 ...
 * 
 */
class HPHtwisting extends MY_Controller
{
	...
	public function loadData()
	{
		...
		if(strtotime($tglsampai) < strtotime($tgldari) ){
			...
			$allcount           = $this->m_HPHtwisting->getRecordCountHPH($where);
	...
	public function export_excel_hph()
	{
		...
		$object->getActiveSheet()->mergeCells('A2:B2');
 		$object->getActiveSheet()->SetCellValue('C2', ': Twisting');
		...
		foreach ($index_header as $val) {
			...
			$object->getSheet(0)->getColumnDimension('H')->SetWidth(12);
			$object->getSheet(0)->getColumnDimension('I')->SetWidth(20);
			$object->getSheet(0)->getColumnDimension('J')->SetWidth(10);
			$object->getSheet(0)->getColumnDimension('K')->SetWidth(9);
    	...
    	foreach ($items as $row) {
    		...
    		if($rowCount > 6){
				...
				$object->getActiveSheet()->SetCellValue('H'.$rowCount, $tpm);
