...
class Rekapkaskecil extends MY_Controller {
...
//        $data["coa"] = $model->setTables("acc_coa")->setWheres(["jenis_transaksi" => "kas"])->setOrder(["kode_coa"])->getData();
        $this->load->view('report/acc/v_rekap_kas_kecil', $data);
    }
    ...
    protected function _query() {
        try {
            ...
            $model = new $this->m_global;
            $model->setTables("acc_kas_kecil_{$kas} ag")->setJoins("acc_kas_kecil_{$kas}_detail agd", "kas_kecil_{$kas}_id = ag.id")
    ...
    public function search() {
        try {
            ...
            $data["kas"] = $kas;
            $html = $this->load->view('report/acc/v_rekap_kas_kecil_detail', $data, true);
    ...
    public function export() {
        try {
            ...
            foreach ($data as $key => $value) {
                ...
                $no += 1;
                $sheet->setCellValue("A{$row}", $no);
                $sheet->setCellValue("B{$row}", $value->no_kkk ?? $value->no_kkm);
                $sheet->setCellValue("C{$row}", $kas);
            ...
            $filename = "Rekap Kas Kecil {$kas} {$tanggal}";
