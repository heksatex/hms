...
class Adjustment extends MY_Controller
{
	...
	function export_excel()
    {
		...
		$column = 0;
		$table_head_columns = array('No','Kode Adjustment','Tanggal','Product','Lot','Qty','UoM','Qty2','UoM2','Qty Move','User','Notes');
		...
		foreach ($head as $hd) {
			...
			$object->getActiveSheet()->SetCellValue('J'.$rowCount, $tot_qty_move);
			...
			foreach ($items as $val) {
				...
				$object->getActiveSheet()->SetCellValue('F'.$rowCount, $val->qty_adjustment);
				...
				$object->getActiveSheet()->SetCellValue('J'.$rowCount, $val->qty_move);
				$object->getActiveSheet()->SetCellValue('K'.$rowCount, $val->nama_user);
				$object->getActiveSheet()->SetCellValue('L'.$rowCount, $val->note);
