...
//            $spreadsheet = $spreadsheet->setActiveSheetIndex(0);
                        ...
                        if ($field2 == "info") {
                            for ($i = 0, $l = count($field22); $i < $l; $i++) {
                                foreach ($field22[$i] as $field3 => $field33) {
                                    ...
                                    $sheet->getStyle($column_excel . '' . $rowCount)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
                                    $sheet->getStyle($column_excel . '' . $rowCount)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                                    $column++;
                                    $column_e++;
                                }
                            }
                            ...
                            foreach ($field22 as $field3) {
                                ...
                                if (count($field3['child']) > 0) {
                                    ...
                                    $sheet->getStyle($column_excel_start . '' . $rowCount)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
                                    $sheet->getStyle($column_excel_start . '' . $rowCount)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                                }
                            }
                            $column_e++;
                            $out_or_in = false;
                                ...
                                foreach ($field22 as $field3) {
                                    ...
                                    if (count($field3['child']) > 0) {
                                        ...
                                        foreach ($field3['child'] as $field4) {// departemen
                                            ...
                                            $sheet->getStyle($column_excel_start2 . '' . $rowCountMerge)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
                                            $sheet->getStyle($column_excel_start2 . '' . $rowCountMerge)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            ...
            if ($view == "Global") {
                $name_file = 'Mutasi Global ' . $dept . ' ' . bln_indo(date('d-m-Y', strtotime($tanggal)));
            } else if ($view == "DetailProduk") {
                $name_file = 'Mutasi Detail Produk ' . $dept . ' ' . bln_indo(date('d-m-Y', strtotime($tanggal)));
            } else {
                $name_file = 'Mutasi Detail Lot ' . $dept . ' ' . bln_indo(date('d-m-Y', strtotime($tanggal)));
            }

            $writer = new Xlsx($spreadsheet);
            $writer = IOFactory::createWriter($spreadsheet, 'Xlsx');
            $writer->save(FCPATH . 'dist/storages/report/suratjalan/' . $name_file . '.xlsx');

            $response = array(
                'op' => 'ok',
                'file' => base_url('dist/storages/report/suratjalan/' . $name_file . '.xlsx'),
                'filename' => $name_file
            );
...
//            $this->load->library('excel');
            ...
            foreach ($table_mutasi as $tm) {
                ...
                $rowCount = $rowCount + 3;
                foreach ($tm['head_table'] as $field) {
                    $sheet->setCellValueByColumnAndRow($column, $rowCount, $field);
                    $column++;
                }
            ...
            $name_file = 'Mutasi Detail Lot ' . $dept['nama'] . ' ' . bln_indo(date('d-m-Y', strtotime($tanggal)));
            $writer = new Xlsx($spreadsheet);
            $writer = IOFactory::createWriter($spreadsheet, 'Xlsx');
            $writer->save(FCPATH . 'dist/storages/report/suratjalan/' . $name_file . '.xlsx');
    ...
    function export_excel_mutasi_detail_($tanggal, $departemen, $kode_produk, $nama_produk, $kode_transaksi, $lot, $reff_picking, $reff_note) {
