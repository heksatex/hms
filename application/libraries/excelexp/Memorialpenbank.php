...
class Memorialpenbank {
    ...
    protected $notList = ['1161.01', '1161.02', '1163.01', '1163.02', '1164.01', '2193.01', '2193.02'];
    ...
    public function _data($model, $datas) {
        ...
        try {
            ...
            $model->setTables("acc_bank_masuk bm")->setJoins("acc_bank_masuk_detail bmd", "bank_masuk_id = bm.id")
                    ...
                    ->setWhereRaw("bmd.kode_coa not in ('{$nt}')")->setSelects(["if(partner_nama ='',lain2,partner_nama) as partner,bm.no_bm,bmd.kurs"])
                    ->setSelects(["bm.kode_coa,bmd.kode_coa as kode_coa_bmd,acbm.nama,acbmd.nama as nama_bmd,sum(bmd.nominal) as valas,sum(bmd.nominal*bmd.kurs) as nominals,date(bmd.tanggal) as tanggal"])
            ...
            $model->setTables("acc_giro_masuk gm")->setJoins("acc_giro_masuk_detail gmd", "giro_masuk_id = gm.id")
                    ...
                    ->setSelects(["gm.kode_coa,acgm.nama,gmd.kode_coa as kode_coa_gmd,acgmd.nama as nama_gmd,sum(gmd.nominal) as valas,sum(gmd.nominal*gmd.kurs) as nominals"])
