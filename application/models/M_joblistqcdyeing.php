 ...
 * 
 */
class M_joblistqcdyeing extends CI_Model
{
	...
	{	
		...
		$this->db->select("mrp.kode, mrp.dept_id, mrp.tanggal, mrp.origin, mrp.kode_produk, mrp.nama_produk, mrp.qty, mrp.uom, mrp.qty1_std, mrp.qty2_std, mrp.status, mmss.nama_status, pb.kode as pb_kode, pb.move_id as pb_move_id, pb.status as pb_status ");
		$this->db->from('mrp_production mrp');		
		$this->db->join("(SELECT * FROM mrp_production where dept_id IN ('".$dept."') AND status IN ('draft', 'ready', 'done') ) as mrp2","mrp2.kode = mrp.kode AND mrp2.dept_id = mrp.dept_id AND mrp2.status = mrp.status","inner");
		...
		$this->db->join("pengiriman_barang pb", "pb.origin = mrp.origin AND pb.dept_id IN ('".$dept."')","inner");
