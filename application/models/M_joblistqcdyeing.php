...
class M_joblistqcdyeing extends CI_Model
{
	...
	{	
		...
		$this->db->join("(SELECT * FROM mrp_production where dept_id = '".$id_dept."' AND status IN ('draft', 'ready', 'done') ) as mrp2","mrp2.kode = mrp.kode AND mrp2.dept_id = mrp.dept_id AND mrp2.status = mrp.status","inner");
