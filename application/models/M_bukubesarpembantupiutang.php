...
class M_bukubesarpembantupiutang extends CI_Model
{
    ...
    function get_saldo_sblm($tgldari, $tipe, string $currency = '')
    {
        ...
        if($tipe == 'kas_um'){
            ...
            $tmp_where = ['app.tanggal_transaksi >= ' => $tgl_dari, 'app.tanggal_transaksi <= ' => $tgl_sampai, 'app.status' => 'done', 'apps.tipe_currency' => 'Rp', 'appsk.lunas'=> 1];
            if ($currency === 'valas') {
                $cr_condition = ($currency === 'valas') ? '<>' : '';
                $tmp_where = array_merge($tmp_where, ['apps.tipe_currency ' . $cr_condition => 'Rp']);
            }
    ...
    function query1() 
    {
        ...
        $this->db->select("abmd.id,(abm.no_bm) as no_bukti, abm.partner_id,  abmd.tanggal, abmd.currency_id, ck1.currency, abmd.kurs,abmd.nominal, 'bank' as tipe2, abmd.uraian, abmd.kode_coa, abm.status");
    ...
    function query2()
    {
        ...
        $this->db->select("akmd.id,(akm.no_km) as no_bukti, akm.partner_id, akmd.tanggal, akmd.currency_id, ck2.currency, akmd.kurs,akmd.nominal, 'kas' as tipe2, akmd.uraian, akmd.kode_coa, akm.status");
    ...
    function query3()
    {
        ...
        $this->db->select("agmd.id,(agm.no_gm) as no_bukti,agm.partner_id, agmd.tanggal, agmd.currency_id, ck3.currency, agmd.kurs,agmd.nominal, 'giro' as tipe2, iF(agm.transinfo !='', agm.transinfo, agm.lain2 ) as uraian, agmd.kode_coa, agm.status");
    ...
    function get_saldo_kas_um(array $where = [], string $group = '', string $currency = '') 
    {
        ...
        $this->db->select("id as id_bukti, no_bukti as id_bukti_ecr, no_bukti,  tanggal as tgl, partner_id, IF(currency_id  != 1, CONCAT('Uang Muka (Oustanding) - ','Kurs : ', kurs) , 'Uang Muka (Oustanding)' ) as uraian, 
    ...
    function get_saldo_diskon_sub(array $where = [], string $group = '', string $currency = '')
    {
        ...
        $total = ($currency === 'valas') ? "IFNULL(ROUND(SUM(CAST(b.diskon AS DECIMAL(20,4))), 2),0) + IFNULL(ROUND(SUM(CAST(b.diskon * 11/12 * a.tax_value AS DECIMAL(20,4))), 2), 0)" : "IFNULL(ROUND(SUM(CAST(b.diskon*a.kurs_nominal AS DECIMAL(20,4))), 0),0) + IFNULL(ROUND(SUM(CAST(b.diskon * 11/12 * a.tax_value AS DECIMAL(20,4))), 0), 0)";
