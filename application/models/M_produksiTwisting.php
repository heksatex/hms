 ...
 * 
 */
class M_produksiTwisting extends CI_Model
{

	public function get_list_produksi_by_dept($where)
	{
		return $this->db->query("SELECT mp.kode, mp.tanggal, mp.nama_produk, mp.qty as qty_target, mp.reff_note, mp.status,ms.nama_mesin,mp.origin,
										...
										(SELECT IFNULL(sum(mpfg.qty2),0) as total_qty2
												 FROM mrp_production_fg_hasil mpfg
												 WHERE mpfg.kode = mp.kode AND mpfg.lokasi NOT IN (SELECT d.waste_location FROM departemen d WHERE d.kode = 'WRD')) as hph_qty2,
												 
										(SELECT IFNULL(mp.qty - IFNULL(sum(mpfg.qty),0),0) 
												 FROM mrp_production_fg_hasil mpfg
												 WHERE mpfg.kode = mp.kode AND mpfg.lokasi NOT IN (SELECT d.waste_location FROM departemen d WHERE d.kode = 'WRD')) as sisa_target
