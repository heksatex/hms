...
class M_produk extends CI_Model {
    ...
    public function get_produk_by_kode($id) {
        return $this->db->query("SELECT mp.id, mp.kode_produk, mp.nama_produk, mp.uom, mp.uom_2, mp.create_date, mp.lebar_greige, 
                    mp.uom_lebar_greige, mp.lebar_jadi, mp.uom_lebar_jadi, mp.route_produksi, mp.type, mp.dapat_dibeli, mp.dapat_dijual, 
                    mp.id_category, mp.bom, mp.note, mp.status_produk, mp.id_parent, mpp.nama as nama_parent, mp.id_jenis_kain, mp.id_sub_parent, 
                    mpsp.nama_sub_parent,mp.image,uom_beli
					 			FROM mst_produk mp
    ...
    public function get_list_uom(array $where = []) {
        $this->db->from("uom");
        if (count($where) > 0) {
            $this->db->where($where);
        }
        $this->db->order_by("short");
        return $this->db->select("short")->get()->result();
    }
    ...
    public function update_produk($id, $nama_produk, $uom, $uom_2, $route_produksi, $type, $dapat_dibeli, $dapat_dijual,
            $id_category, $note, $bom, $lebargreige, $uom_lebargreige, $lebarjadi, $uom_lebarjadi,
            $statusproduk, $product_parent, $sub_parent, $jenis_kain, $uom_beli = null) {
        return $this->db->query("UPDATE mst_produk set nama_produk = '$nama_produk', uom = '$uom',uom_2 = '$uom_2',"
                        . " route_produksi = '$route_produksi', type = '$type', dapat_dibeli = '$dapat_dibeli', dapat_dijual = '$dapat_dijual',"
                        . " id_category = '$id_category', note = '$note', bom = '$bom', lebar_greige = '$lebargreige',"
                        . " uom_lebar_greige = '$uom_lebargreige' ,lebar_jadi = '$lebarjadi', uom_lebar_jadi = '$uom_lebarjadi',"
                        . " status_produk = '$statusproduk', id_parent = '$product_parent',  id_sub_parent = '$sub_parent',"
                        . " id_jenis_kain ='$jenis_kain',uom_beli='{$uom_beli}' WHERE id = '$id' ");
    }
    ...
    public function getCatatan($id) {
        $this->db->from("mst_produk_catatan");
