...
class M_joblistgrgout extends CI_Model
{
	...
	var $column_order = array(null, 'pb.kode', 'tanggal', 'pb.origin', 'reff_picking', null, 'pbi.nama_produk',  'target_mtr', 'mtr', 'kg', 'gl', 'nama_status','nama_warna');
	...
	{	
		...
		INNER JOIN warna as w ON cod.id_warna = w.id
			WHERE cod.kode_co = SUBSTRING_INDEX(SUBSTRING_INDEX(pb.origin,'|',2),'|',-1) AND cod.row_order  = SUBSTRING_INDEX(SUBSTRING_INDEX(pb.origin,'|',3),'|',-1)) as nama_warna, (select delivery_date FROM sales_contract where sales_order = SUBSTRING_INDEX(pb.origin, '|', 1) ) as delivery_date ");
