...
class M_reportAdjustment extends CI_Model
{
	...
	function craete_where($kode_adjustment,$nama_produk,$type_adjustment,$user,$notes)
	{
		$where = '';
		if($kode_adjustment){
	...
	public function get_list_group_nama_produk_adj_in_by_kode($lokasi, $tgl_dari, $tgl_sampai,$kode_adjustment,$nama_produk,$type_adjustment,$user,$notes)
	{
		$where = $this->craete_where($kode_adjustment,$nama_produk,$type_adjustment,$user,$notes);
	...
	public function get_list_group_nama_produk_adj_out_by_kode($lokasi, $tgl_dari, $tgl_sampai,$kode_adjustment,$nama_produk,$type_adjustment,$user,$notes)
	{
		$where = $this->craete_where($kode_adjustment,$nama_produk,$type_adjustment,$user,$notes);
		return $this->db->query("SELECT b.kode_produk, mp.nama_produk, count(lot) as tot_lot, IFNULL(sum(b.qty_move),0) as tot_qty_move , IFNULL(sum(b.qty_data),0) tot_qty_stock, IFNULL(sum(b.qty_adjustment),0) tot_qty1_adj,  IFNULL(sum(b.qty_data2),0) tot_qty2_stock, IFNULL(sum(b.qty_adjustment2),0) tot_qty2_adj, mp.uom,mp.uom_2, IFNULL(sum(b.qty2_move),0) as tot_qty2_move 
								FROM adjustment_items b 
	...
	public function get_list_item_adjustment_by_kode($kode_lokasi,$tgldari,$tglsampai,$data_isi,$where_adj,$kode_adjustment,$nama_produk,$type_adjustment,$user,$notes)
	{
		$where = $this->craete_where($kode_adjustment,$nama_produk,$type_adjustment,$user,$notes);
