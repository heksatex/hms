...
#CLEAR
DELETE FROM acc_mutasi_wrd_rm
WHERE periode_th = YEAR(SUBDATE$replace_currDate) AND periode_bln = MONTH(SUBDATE$replace_currDate);

#SALDO AWAL
INSERT INTO acc_mutasi_wrd_rm (periode_th, periode_bln, dept_id_mutasi, kode_produk, nama_produk, s_awal_lot, s_awal_qty1, s_awal_qty1_uom, s_awal_qty2, s_awal_qty2_uom, s_awal_qty_opname, s_awal_qty_opname_uom)
...
AND sq.lokasi='WRD/Stock'
AND mp.id_category NOT IN (SELECT id FROM mst_category WHERE dept_id='WRD')
...
BEGIN
	...
	loop_label:  LOOP
		...
		INSERT IGNORE INTO acc_mutasi_wrd_rm (periode_th, periode_bln, dept_id_mutasi, kode_produk, nama_produk)
		SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
		'WRD' as dept_id_mutasi,
		smi.kode_produk, smi.nama_produk
		FROM acc_stock_move_items smi 
		INNER JOIN (SELECT * FROM acc_dept_mutasi WHERE step='rm') dm ON smi.dept_id_mutasi=dm.dept_id AND dm.seq=seq_
		WHERE smi.dept_id_mutasi = 'WRD'
		AND smi.type = dm.type
		AND smi.id_category IN (SELECT id FROM mst_category WHERE dept_id<>'WRD')
		...
		SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
		'WRD' as dept_id_mutasi, smi.kode_produk, smi.nama_produk,
		COUNT(*) as total_lot,
...
SET
s_akhir_lot = s_awal_lot + in1_lot + in2_lot + in3_lot + in4_lot + in5_lot + adj_in_lot - (out1_lot + out2_lot + out3_lot + out4_lot + out5_lot + out6_lot + out7_lot + adj_out_lot),
s_akhir_qty1 = s_awal_qty1 + in1_qty1 + in2_qty1 + in3_qty1 + in4_qty1 + in5_qty1 + adj_in_qty1 - (out1_qty1 + out2_qty1 + out3_qty1 + out4_qty1 + out5_qty1 + out6_qty1 + out7_qty1 + adj_out_qty1),
s_akhir_qty1_uom = b.uom,
s_akhir_qty2 = s_awal_qty2 + in1_qty2 + in2_qty2 + in3_qty2 + in4_qty2 + in5_qty2 + adj_in_qty2 - (out1_qty2 + out2_qty2 + out3_qty2 + out4_qty2 + out5_qty2 + out6_qty2 + out7_qty2 +  adj_out_qty2),
s_akhir_qty2_uom = b.uom_2,
s_akhir_qty_opname = s_awal_qty_opname + in1_qty_opname + in2_qty_opname + in3_qty_opname + in4_qty_opname + in5_qty_opname + adj_in_qty_opname - (out1_qty_opname + out2_qty_opname + out3_qty_opname + out4_qty_opname + out5_qty_opname + out6_qty_opname + out7_qty_opname + adj_out_qty_opname),
...
VALUE (now(), 'UPDATE|acc_mutasi_wrd_rm|Proses Koreksi Mundur');
