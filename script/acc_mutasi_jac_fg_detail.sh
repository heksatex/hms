...
#CLEAR
DELETE FROM acc_mutasi_jac_fg_detail
WHERE periode_th = YEAR(SUBDATE$replace_currDate) AND periode_bln =MONTH(SUBDATE$replace_currDate);

#SALDO AWAL
INSERT INTO acc_mutasi_jac_fg_detail (periode_th, periode_bln, posisi_mutasi, dept_id_mutasi, dept_id_dari, dept_id_tujuan, type, kode_transaksi, tanggal_transaksi, kode_produk, nama_produk, id_category, lot, qty, uom, qty2, uom2, qty_opname, uom_opname, origin, source_move, method, reff_note, sc, sales_group, mo)
...
#IN & ADJ_IN
INSERT INTO acc_mutasi_jac_fg_detail (periode_th, periode_bln, posisi_mutasi, dept_id_mutasi, dept_id_dari, dept_id_tujuan, type, kode_transaksi, tanggal_transaksi, kode_produk, nama_produk, id_category, lot, qty, uom, qty2, uom2, qty_opname, uom_opname, origin, source_move, method, reff_picking, sc, sales_group, mo, reff_note)
SELECT YEAR(SUBDATE$replace_currDate) as periode_th,MONTH(SUBDATE$replace_currDate) as periode_bln,
...
VALUE (now(), 'INSERT|acc_mutasi_jac_fg_detail|Proses Koreksi Mundur');
