...
#CLEAR
DELETE FROM acc_mutasi_tri_fg
WHERE periode_th = YEAR(SUBDATE$replace_currDate) AND periode_bln = MONTH(SUBDATE$replace_currDate);

#SALDO AWAL
INSERT INTO acc_mutasi_tri_fg (periode_th, periode_bln, dept_id_mutasi, kode_produk, nama_produk, s_awal_lot, s_awal_qty1, s_awal_qty1_uom, s_awal_qty2, s_awal_qty2_uom, s_awal_qty_opname, s_awal_qty_opname_uom)
...
sq.kode_produk, sq.nama_produk, 
COUNT(*) as total_lot,
COALESCE(SUM(sq.qty),0) as qty1, COALESCE(sq.uom,'') as qty1_uom, 
COALESCE(SUM(sq.qty2),0) as qty2, COALESCE(sq.uom2,'') as qty2_uom2, 
COALESCE(SUM(sq.qty_opname),0) as qty_opname, COALESCE(sq.uom_opname,'') as uom_opname
...
AND sq.lokasi='TRI/Stock'
AND mp.id_category IN (SELECT id FROM mst_category WHERE dept_id='TRI')
...
BEGIN
	...
	loop_label:  LOOP
		...
		INSERT IGNORE INTO acc_mutasi_tri_fg (periode_th, periode_bln, dept_id_mutasi, kode_produk, nama_produk)
		SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
		'TRI' as dept_id_mutasi,
		smi.kode_produk, smi.nama_produk
		FROM acc_stock_move_items smi 
		INNER JOIN (SELECT * FROM acc_dept_mutasi WHERE step='fg') dm ON smi.dept_id_mutasi=dm.dept_id AND dm.seq=seq_
		WHERE smi.dept_id_mutasi = 'TRI'
		AND smi.type = dm.type
		AND smi.id_category IN (SELECT id FROM mst_category WHERE dept_id='TRI')
		...
		SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
		'TRI' as dept_id_mutasi, smi.kode_produk, smi.nama_produk,
		COUNT(*) as total_lot,
...
#OUT ambil dari GRG/IN karena tidak ada TRI/OUT
#OUT insert ignore corak
	INSERT IGNORE INTO acc_mutasi_tri_fg (periode_th, periode_bln, dept_id_mutasi, kode_produk, nama_produk)
	SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
	'TRI' as dept_id_mutasi,
	...
	AND smi.dept_id_dari = 'TRI'
	AND smi.type = 'in'
	AND smi.method = 'GRG|IN'
	AND mp.id_category IN (SELECT id FROM mst_category WHERE dept_id='TRI')
	AND smi.tanggal_transaksi >= CONCAT(date_add(date_add(LAST_DAY(SUBDATE$replace_currDate),interval 1 DAY),interval -1 MONTH), ' 00:00:000')
	AND smi.tanggal_transaksi <= CONCAT(SUBDATE$replace_currDate,' 23:59:59')
	GROUP BY kode_produk;
	...
	SUM(smi.qty_opname) as qty_opname, COALESCE(smi.uom_opname,'') as opname_uom,
	'in' as seq
	FROM acc_stock_move_items smi
	INNER JOIN mst_produk mp ON smi.kode_produk = mp.kode_produk
	WHERE smi.dept_id_mutasi = 'GRG'
	...
	AND smi.tanggal_transaksi <= CONCAT(SUBDATE$replace_currDate,' 23:59:59')
	GROUP BY smi.kode_produk;
	#update mutasi hardcode selalu ke out1
	UPDATE acc_mutasi_tri_fg a
	INNER JOIN acc_mutasi_tmp b
	ON a.periode_th = b.periode_th AND a.periode_bln = b.periode_bln AND a.dept_id_mutasi = b.dept_id_mutasi AND a.kode_produk = b.kode_produk
	SET
	a.out1_lot=a.out1_lot+b.lot,
	a.out1_qty1=a.out1_qty1+b.qty1,
	a.out1_qty1_uom=b.qty1_uom,
	a.out1_qty2=a.out1_qty2+b.qty2,
	a.out1_qty2_uom=b.qty2_uom,
	a.out1_qty_opname=a.out1_qty_opname+b.qty_opname,
	a.out1_qty_opname_uom=b.qty_opname_uom;
	#delete temp
	DELETE FROM acc_mutasi_tmp;
...
SET
s_akhir_lot = s_awal_lot + in1_lot + in2_lot + in3_lot + in4_lot + in5_lot + adj_in_lot - (out1_lot + out2_lot + out3_lot + out4_lot + out5_lot + adj_out_lot),
s_akhir_qty1 = s_awal_qty1 + in1_qty1 + in2_qty1 + in3_qty1 + in4_qty1 + in5_qty1 + adj_in_qty1 - (out1_qty1 + out2_qty1 + out3_qty1 + out4_qty1 + out5_qty1 + adj_out_qty1),
s_akhir_qty1_uom = b.uom,
s_akhir_qty2 = s_awal_qty2 + in1_qty2 + in2_qty2 + in3_qty2 + in4_qty2 + in5_qty2 + adj_in_qty2 - (out1_qty2 + out2_qty2 + out3_qty2 + out4_qty2 + out5_qty2 + adj_out_qty2),
s_akhir_qty2_uom = b.uom_2,
s_akhir_qty_opname = s_awal_qty_opname + in1_qty_opname + in2_qty_opname + in3_qty_opname + in4_qty_opname + in5_qty_opname + adj_in_qty_opname - (out1_qty_opname + out2_qty_opname + out3_qty_opname + out4_qty_opname + out5_qty_opname + adj_out_qty_opname),
...
VALUE (now(), 'UPDATE|acc_mutasi_tri_fg|Proses Koreksi Mundur');
