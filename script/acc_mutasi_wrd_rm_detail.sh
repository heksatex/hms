...
#CLEAR
DELETE FROM acc_mutasi_wrd_rm_detail
WHERE periode_th = YEAR(SUBDATE$replace_currDate) AND periode_bln = MONTH(SUBDATE$replace_currDate);

#SALDO AWAL
INSERT INTO acc_mutasi_wrd_rm_detail (periode_th, periode_bln, posisi_mutasi, dept_id_mutasi, dept_id_dari, dept_id_tujuan, type, kode_transaksi, tanggal_transaksi, kode_produk, nama_produk, id_category, lot, qty, uom, qty2, uom2, qty_opname, uom_opname, origin, source_move, method, reff_note, sc, sales_group, mo)
SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
'1.SALDO AWAL' as posisi_mutasi, 'WRD' as dept_id_mutasi, 'WRD' as dept_id_dari, 'WRD' as dept_id_tujuan, '' as type, '' as kode_transaksi, sq.tanggal as tanggal_transaksi,
...
#IN & ADJ_IN
INSERT INTO acc_mutasi_wrd_rm_detail (periode_th, periode_bln, posisi_mutasi, dept_id_mutasi, dept_id_dari, dept_id_tujuan, type, kode_transaksi, tanggal_transaksi, kode_produk, nama_produk, id_category, lot, qty, uom, qty2, uom2, qty_opname, uom_opname, origin, source_move, method, reff_picking, sc, sales_group, mo, reff_note)
SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
...
AND smi.id_category NOT IN (SELECT id FROM mst_category WHERE dept_id='WRD')
...
WHERE smi.dept_id_mutasi = 'WRD'
AND smi.id_category NOT IN (SELECT id FROM mst_category WHERE dept_id='WRD')
AND smi.type IN ('out', 'adj_out', 'con')
AND smi.tanggal_transaksi >= CONCAT(date_add(date_add(LAST_DAY(SUBDATE$replace_currDate),interval 1 DAY),interval -1 MONTH), ' 00:00:000')
AND smi.tanggal_transaksi <= CONCAT(SUBDATE$replace_currDate,' 23:59:59');
...
VALUE (now(), 'INSERT|acc_mutasi_wrd_rm_detail|Proses Koreksi Mundur');
