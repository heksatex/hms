...
END) "in5_kg",
SUM(m.in6_lot) in6_lot, SUM(CASE WHEN mp.uom='Mtr' THEN m.in6_qty1 END) "in6_mtr",
SUM(CASE 
	WHEN mp.uom='Kg' THEN m.in6_qty1 
	WHEN mp.uom_2='Kg' THEN m.in6_qty2
END) "in6_kg",
...
END) "out5_kg",
SUM(m.out6_lot) out6_lot, SUM(CASE WHEN mp.uom='Mtr' THEN m.out6_qty1 END) "out6_mtr",
SUM(CASE 
	WHEN mp.uom='Kg' THEN m.out6_qty1 
	WHEN mp.uom_2='Kg' THEN m.out6_qty2
END) "out6_kg",
SUM(m.out7_lot) out7_lot, SUM(CASE WHEN mp.uom='Mtr' THEN m.out7_qty1 END) "out7_mtr",
SUM(CASE 
	WHEN mp.uom='Kg' THEN m.out7_qty1 
	WHEN mp.uom_2='Kg' THEN m.out7_qty2
END) "out7_kg",
SUM(m.out8_lot) out8_lot, SUM(CASE WHEN mp.uom='Mtr' THEN m.out8_qty1 END) "out8_mtr",
SUM(CASE 
	WHEN mp.uom='Kg' THEN m.out8_qty1 
	WHEN mp.uom_2='Kg' THEN m.out8_qty2
END) "out8_kg",
...
VALUE (now(), 'UPDATE|acc_mutasi_grg_global|Proses Koreksi Mundur');
