...
#CLEAR
DELETE FROM acc_mutasi_wrd_fg
WHERE periode_th = YEAR(SUBDATE$replace_currDate) AND periode_bln = MONTH(SUBDATE$replace_currDate);

#SALDO AWAL
INSERT INTO acc_mutasi_wrd_fg (periode_th, periode_bln, dept_id_mutasi, kode_produk, nama_produk, s_awal_lot, s_awal_qty1, s_awal_qty1_uom, s_awal_qty2, s_awal_qty2_uom, s_awal_qty_opname, s_awal_qty_opname_uom)
...
AND mp.id_category IN (SELECT id FROM mst_category WHERE dept_id='WRD')
...
BEGIN
	...
	loop_label:  LOOP
		...
		INSERT IGNORE INTO acc_mutasi_wrd_fg (periode_th, periode_bln, dept_id_mutasi, kode_produk, nama_produk)
		SELECT YEAR(SUBDATE$replace_currDate) as periode_th, MONTH(SUBDATE$replace_currDate) as periode_bln,
...
VALUE (now(), 'UPDATE|acc_mutasi_wrd_fg|Proses Koreksi Mundur');
